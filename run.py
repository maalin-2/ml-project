...
# evaluates logistic risk and binary classification error on the testing set
def predictor_test(predictor, testing_set):
    w = predictor
    total_loss = 0.0
    total_error = 0

    # accumulate loss and misclassifications over the test set
...
# runs all experiments across parameter settings and prints results
def main():
    settings = {0.2: [], 0.4: []}
    test_sets = {sigma: dg.generate_testing_set(sigma) for sigma in [0.2, 0.4]}

    # run each (n, sigma) configuration and collect results
